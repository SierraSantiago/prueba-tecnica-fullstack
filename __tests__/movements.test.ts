...
describe("Movements API", () => {
  ...
  test("GET /api/movements returns 200 with list of movements", async () => {
    const mockMovements = [
      { id: 1, concept: "Compra", amount: 50, date: new Date().toISOString(), user: { id: "1", name: "Santiago" } },
      { id: 2, concept: "Venta", amount: 150, date: new Date().toISOString(), user: { id: "2", name: "Juan" } },
    ];
    (MovementService.getAllMovements as jest.Mock).mockResolvedValue(mockMovements);
  ...
  test("POST /api/movements creates a new movement", async () => {
    const newMovement = { concept: "Test", amount: 100, date: new Date().toISOString(), userId: "user_123" };
    const createdMovement = { ...newMovement, id: 1, user: { id: "user_123", name: "Santiago" } };

    (MovementService.createMovement as jest.Mock).mockResolvedValue(createdMovement);
  ...
  test("POST /api/movements fails if missing fields", async () => {
    ...
    const { req, res } = createMocks({ method: "POST", body: incompleteMovement });
    await expect(createMovement(req as any, res as any)).rejects.toThrow("Missing fields");
  });
});
